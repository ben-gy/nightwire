...
describe('createListing', () => {
  it('announces only when the room is public', () => {
    ...
    expect(board.ads[0].note).toBe('Standard');
  });
  ...
  it('carries the live player count and the mode note', () => {
    ...
    const listing = createListing(board);
    listing.sync(LOBBY);
    listing.sync({ ...LOBBY, players: 3, note: 'Inquest' });
...
describe('room entry — browsing is opt-in', () => {
  ...
  it('lists host, note, players and whether a round is running', async () => {
    ...
    await vi.waitFor(() => expect(board.opens).toBe(1));
    board.push([
      { code: 'ABCD', host: 'Ann', players: 2, max: 10, playing: false, note: 'Standard', seenAt: 0 },
      { code: 'WXYZ', host: 'Bo', players: 4, max: 10, playing: true, note: 'Blackout', seenAt: 0 },
    ]);
    ...
    expect(rows[0].textContent).toContain('2/10');
  ...
  it('does not call the list empty before the mesh has had time to form', async () => {
    ...
    board.push([{ code: 'ABCD', host: 'Ann', players: 1, max: 10, playing: false, seenAt: 0 }]);
  ...
  it('joins a room off the list as a GUEST, never claiming host', async () => {
    ...
    await vi.waitFor(() => expect(board.opens).toBe(1));
    board.push([
      { code: 'ABCD', host: 'Ann', players: 2, max: 10, playing: false, seenAt: 0 },
    ]);
    click('.re-room');
