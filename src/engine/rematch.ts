 ...
 *
 * Nightwire note: this owns the channels 'rv', 'rs' and 'rq'. The in-game role
 * re-attest uses 'rrq'/'rrl' precisely so it cannot collide with 'rq' — since
 * net.channel() now fans out, two subsystems sharing a name both receive, and a
 * rematch resync would otherwise make every peer post its secret role.
 */
